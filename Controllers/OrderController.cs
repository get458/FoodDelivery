...
public class OrderController : ControllerBase
{
    ...
    public IActionResult CreateOrderFromBasket(OrderCreateDto orderCreateDto)
    {
        if ((orderCreateDto.DeliveryTime - DateTime.Now).TotalMinutes < 60)
        {
            ...
            {
                Message = "Неверный срок доставки. Время доставки должно превышать текущую дату и время на 60 минут"
            });
        }
        try
        {
        ...
        catch (EmptyBasketException e)
        {
            return BadRequest(new
            {
                Message = $"Пустая корзина для пользователя с ID = {e.UserId}"
            });
        }
    }
    ...
    public IActionResult ConfirmOrderDelivery(Guid id)
    {
        ...
        catch (OrderNotFoundException)
        {
            return NotFound(new
            {
                Message = $"Заказ с ID ={id} не в корзине"
            });
        }
        catch (OrderConfirmedException)
        {
            return BadRequest(new
            {
                Message = $"Не удается обновить статус заказа с ID = {id}"
            });
        }
        catch (ForbiddenException)
        {
